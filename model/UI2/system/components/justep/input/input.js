	...
	var Input = justep.BindComponent.extend({
		...
		_unbindDataType : function(dataType) {
			if (this.$domNode) {
				...
				if (dataType == 'DateTime' || dataType == 'Date' || dataType == 'Time'){
					this.$domNode.attr('readonly', this.readonly).datePicker('destroy');
				}
			}
		},
		...
		doInit : function(value, bindingContext) {
			...
			this.$domNode.on('change', $.proxy(this.doChange, this)).on('focus', function() {
				...
				if(!self._refocus && justep.Browser.isIOS && !justep.Browser.isX5App) window.setTimeout(function(){self._refocus = true; self.$domNode.blur().focus();},1);
				self._refocus = false;
			}).on('blur', function() {
				self._focusin = false;
				if(!self._refocus)self._restoreType();
				self.render();
